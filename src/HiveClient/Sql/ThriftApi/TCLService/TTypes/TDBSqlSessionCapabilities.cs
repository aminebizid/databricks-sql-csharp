...
namespace HiveClient.Sql.ThriftApi.TCLService.TTypes
{
  ...
  public partial class TDBSqlSessionCapabilities : TBase
  {
    ...
    {
      if (that is not TDBSqlSessionCapabilities other) return false;
    ...
    {
      var tmp218 = new StringBuilder("TDBSqlSessionCapabilities(");
      int tmp219 = 0;
      if(__isset.supportsMultipleCatalogs)
      {
        if(0 < tmp219++) { tmp218.Append(", "); }
        tmp218.Append("SupportsMultipleCatalogs: ");
        SupportsMultipleCatalogs.ToString(tmp218);
      }
      tmp218.Append(')');
